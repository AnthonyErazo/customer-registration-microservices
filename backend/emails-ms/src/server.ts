...
import { setupQueueConsumer, closeQueueConsumer } from './services/queue.service';
...
/**
 * Initializes external services (RabbitMQ consumer)
 * @returns Promise<void>
 * @throws Error if RabbitMQ setup fails
 */
async function initializeServices(): Promise<void> {
  try {
    await setupQueueConsumer();
    fastify.log.info('RabbitMQ consumer connected successfully');
  } catch (error) {
    fastify.log.error({ error }, 'RabbitMQ setup failed');
    throw error;
  }
}
...
async function start(): Promise<void> {
  try {
    ...
    fastify.log.info({
      message: 'Emails microservice started successfully',
